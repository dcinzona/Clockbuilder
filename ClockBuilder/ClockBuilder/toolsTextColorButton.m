...
@implementation toolsTextColorButton
@synthesize pickerAS,
fontButtonLabel;
...
- (IBAction)changeColor: (id)sender {  
    ...
    [self.window.rootViewController performSelector:@selector(updateTextColor:) withObject:newColor];    
}
...
-(void)fontButtonClick:(id)sender
{
    ...
    if(kIsIpad){
        ...
        [vc setContentSizeForViewInPopover:CGSizeMake(320, 200)];
...
-(void)dismissActionSheet{
    ...
    [self.window.rootViewController performSelector:@selector(updateTextColor:) withObject:color];        
}
...
-(void)saveActionSheet{
    ...
    UIColor *newColor = [UIColor colorWithRed:sliderR.value green:sliderG.value blue:sliderB.value alpha:sliderA.value];
    [self.window.rootViewController performSelector:@selector(saveTextColor:) withObject:newColor];        
}
