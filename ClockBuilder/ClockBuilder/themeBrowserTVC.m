...
@implementation themeBrowserTVC
...
- (id)initWithStyle:(UITableViewStyle)style
{
    ...
    if (self) {
        ...
        categories = [NSArray arrayWithArray:[[[NSUserDefaults standardUserDefaults] objectForKey:@"settings"] objectForKey:@"categoriesArray"]];
        
        NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
        currentCategory = [defaults objectForKey:@"themeBrowserCategory"];
        if(currentCategory == nil || [currentCategory isEqualToString:@""])        
            currentCategory = [categories objectAtIndex:0];
        [defaults setObject:currentCategory forKey:@"themeBrowserCategory"];
        ...
        UIButton *tag = [[UIButton alloc] initWithFrame:CGRectMake(280, 10, 30, 30)];
        UIImageView *tagi = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"tag.png"]];
        [tag addTarget:self action:@selector(selectCategory) forControlEvents:UIControlEventTouchUpInside];
        [tag addSubview:tagi];
        [actionButton setCustomView:tag];
        [tag release];
        [tagi release];
        ...
        [titleLabel setTitle:[NSString stringWithFormat:@"%@ Themes", currentCategory] forState:UIControlStateNormal];
        titleLabel.frame = CGRectMake(0,-2, 150, 44);
        titleLabel.titleLabel.font = [UIFont boldSystemFontOfSize:18];
        [[titleLabel titleLabel] setAdjustsFontSizeToFitWidth:YES];
        //[titleLabel addTarget:self action:@selector(selectCategory) forControlEvents:UIControlEventTouchUpInside];
...
-(void)selectCategory
{
    ...
    [picker showWithTitle:@"Select Category" andPickerList:[NSArray arrayWithArray:[[[NSUserDefaults standardUserDefaults] objectForKey:@"settings"] objectForKey:@"categoriesArray"]]];
}
...
{    
    if(!_themesListRefreshing){
        ...
        NSString *urlString = [NSString stringWithFormat:@"http://clockbuilder.gmtaz.com/getThemes.php?cat=%@&device=iPhone&api=SDFB52f4vw9230V45gdfg&v=%@",currentCategory, version];   
        ...
        dispatch_async(queue, ^{
            if([hconn deviceIsConnectedToInet])
            {
                if(_valid){
                    ...
                    _appCurrent = [hconn checkAppVersion];
                    if(_appCurrent)
                    {
...
-(void)showActionMenu
{
    ...
    UIActionSheet *actionButtonSheet;
        actionButtonSheet = [[UIActionSheet alloc] initWithTitle:@"" delegate:self cancelButtonTitle:@"Cancel" destructiveButtonTitle:nil otherButtonTitles:@"Download", @"Download and Activate", nil];   
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
	...
	else if([title isEqualToString:@"OK"])
	{
        ...
        if(fieldVal != nil && ![fieldVal isEqualToString:@""]) {
            [self saveThemeToLocalWithName:fieldVal];
        }
        else
        {
            [self showAlertWithTextFieldAndTitle:@"Name cannot be blank" andMessage:@"Please enter a new name" andConfirmButtonText:@"OK"];
