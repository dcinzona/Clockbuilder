...
if($api == 'SDFB52f4vw9230V45gdfg' &&  getValidVersion($v))
{
	...
	if($cat=='' || $cat==null)
		$query = mysql_query("SELECT * FROM themes WHERE device='".$device."' ORDER BY themeName ASC");
	else
		$query = mysql_query("SELECT * FROM themes WHERE category='".$cat."' AND device='".$device."' ORDER BY themeName ASC");
